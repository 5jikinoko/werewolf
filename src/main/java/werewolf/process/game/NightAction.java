...
public class NightAction{
    ...
    public Message seerAction(UUID userUUID,UUID targetUUID){ //占い処理
        ...
        if(targetUUID == null){ //ランダム占い
            targetUUID = playersStatus.getRandomUser(playersStatus.getDeadPlayerUUIDs());
        }
    ...
    public Message werewolfAction(UUID userUUID,UUID targetUserUUID,int priority){ //人狼処理
        ...
        if(getWerewolfPlayerUUIDs.contains(targetUserUUID)){ //人狼を襲えない
        ...
        if(!killNominee.containsKey(null)){ //噛み投票
    ...
    public List<UUID> finishNightAction(){ //未完
        ...
        if(!guardTarget.containsValue(maxValueUUID)){   //守られてなければ死ぬ
            playersStatus.alive = false;
        }
        //tonightVictimに存在するUUIDが指すプレイヤーのaliveをfalseにする。
        //今回aliveをfalseにしたプレイヤーのUUID一覧をLis
        lastNightGuardTarget.put(guardTarget.getKey(),guardTarget,getValue());
        guardTarget = new HashMap<UUID,UUID>();
        killNominee = new HashMap<UUID,Integer>();
        tonightVictim = new ArrayList<UUID>();
