...
class NightAction{
    ...
    public Message seerAction(UUID userUUID,UUID targetUUID){
        ...
        if(playersStatus.role.equals(werewolves) || playersStatus.role.equals(blackKnights)){
            message.text = "人狼だった";
        }else if(playersStatus.role.equals(foxSpirits)){
            tonightVictim.add(targetUUID);
            message.text = "人狼でなかった";
        }else{
            message.text = "人狼でなかった";
        }
        if(targetUUID == null){ //未完
    ...
    public Message necromancerAction(UUID userUUID,UUID targetUUID){
        Message message = new Message();
        userUUID = targetUUID;
        if(playerStatus.role.equals(werewolves)){
            message.text = "人狼だった";
        }else{
            message.text = "人狼でなかった";
        }
